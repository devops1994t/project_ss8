...
class configure:
    ...
    def namespace_exist(self, namespace):
        output = run_cmd("kubectl get namespace | grep -w "+str(namespace), custom_error_output="Error running kubectl command 'kubectl get namespace'")
        output_list = output.split()
        if(namespace in output_list):
            if("Active" in output_list):
                self.logging.debug("Namespace "+namespace+" exist")
                return True
            else:
                self.logging.error("Namespace "+namespace+" is not Active")
        else:
            self.logging.info("Namespace does not exist: "+namespace)
        return False
    ...
    def create_namespace(self, namespace):
        self.logging.info("Creating namespace "+namespace)
        run_cmd("kubectl create namespace "+namespace, custom_error_output="Error creating namespace '"+namespace+"'")
...
#        print(self.config)
        for quota_dict in self.config["resourceQuota"]:
            ...
            for namespace in quota_dict.keys():
                if (not namespace_exist(namespace)):
